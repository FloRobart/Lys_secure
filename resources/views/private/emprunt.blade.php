                ...
                @if (isset($emprunts))
                    @foreach ($emprunts as $emprunt)
                        <tr class="tableRow smallText text-center">
                            <!-- Date de l'emprunt -->
                            <td class="tableCell">{{ strftime('%A %d %B %Y', strtotime($emprunt->date_debut)) }}</td>
                            ...
                            <td class="smallRowCenterContainer px-1 min-[460px]:px-2 min-[500px]:px-4 py-2">
                                <!-- Modifier -->
                                <button onclick="editEmprunt('{{ strftime('%Y-%m-%d', strtotime($emprunt->date_debut)) }}', '{{ strftime('%Y-%m-%d', strtotime($emprunt->date_fin)) }}', '{{ $emprunt->nom_actif }}', '{{ $emprunt->banque }}', '{{ $emprunt->montant_transaction }}', '{{ $emprunt->mensualite }}', , '{{ $emprunt->taux_interet_annuel }}', '{{ $emprunt->id }}')" class="smallRowCenterContainer w-fit smallTextReverse font-bold bgBleuLogo hover:bgBleuFonce focus:normalScale rounded-lg min-[500px]:rounded-xl py-1 px-1 min-[500px]:px-2">
