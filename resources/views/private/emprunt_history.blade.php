        ...
        <table class="w-full mt-2">
            ...
            <thead class="w-full">
                <tr class="tableRow smallText text-center font-bold">
                    ...
                    <th class="tableCell" title="Trier les emprunts par date de souscription"        ><a href="{{ URL::current() . '?sort=date_transaction' . '&order=' . $order }}" class="link">Date transaction</a></th>
                    ...
                    <th class="tableCell" title="Trier les emprunts par montant emprunté"            ><a href="{{ URL::current() . '?sort=montant_transaction' . '&order=' . $order }}" class="link">Montant transaction</a></th>
                ...
                @if (isset($emprunts))
                    @foreach ($emprunts as $emprunt)
                        <tr class="tableRow smallText text-center">
                            <!-- Date de l'emprunt -->
                            <td class="tableCell">{{ strftime('%A %d %B %Y', strtotime($emprunt->date_transaction)) }}</td>
                            ...
                            <td class="smallRowCenterContainer px-1 min-[460px]:px-2 min-[500px]:px-4 py-2">
                                <!-- Modifier -->
                                <button onclick="editEmpruntHistory('{{ strftime('%Y-%m-%d', strtotime($emprunt->date_transaction)) }}', '{{ $emprunt->nom_actif }}', '{{ $emprunt->banque }}', '{{ $emprunt->montant_transaction }}', '{{ $emprunt->id }}')" class="smallRowCenterContainer w-fit smallTextReverse font-bold bgBleuLogo hover:bgBleuFonce focus:normalScale rounded-lg min-[500px]:rounded-xl py-1 px-1 min-[500px]:px-2">
                                ...
                                <!-- Supprimer -->
                                <a href="{{ route('emprunt_history.remove', $emprunt->id) }}" onclick="return confirm('Êtes-vous sûr de vouloir supprimer la transaction de l\'emprunt à {{ $emprunt->nom_actif }} du {{ strftime('%A %d %B %Y',strtotime($emprunt->date_transaction)) }} ? Cette action est irréversible.')" class="smallRowCenterContainer w-fit smallTextReverse font-bold bgError hover:bgErrorFonce focus:normalScale rounded-lg min-[500px]:rounded-xl py-1 px-1 min-[500px]:px-2 ml-1 min-[500px]:ml-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="tinySizeIcons">
        ...
        <form id="form" action="{{ route('emprunt_history.add') }}" method="POST" class="rowStartContainer hidden">
            ...
            <div class="colCenterContainer">
                <div class="colStartContainer sm:rowStartContainer">
                    <input id="date_transaction"    name="date_transaction"    required type="date" value="{{ date('Y-m-d') }}"                                 class="w-[55%] sm:w-4/12 mx-2 min-[500px]:mx-4 my-2 text-center inputForm smallText">
